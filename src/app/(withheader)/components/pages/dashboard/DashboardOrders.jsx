...
import { getOrder } from '@/app/(withheader)/api-fetching/order handler/orderHandler';
      ...
      <div className="overflow-x-auto rounded-3xl border border-accent-dark/50">
        <table className="w-full text-left">
          <thead className="bg-accent-dark/30">
            <tr>
              <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-widest text-secondary">Order Id</th>
          ...
          <tbody className="divide-y divide-accent-dark/50">
            {orders.map((order) => (
              <tr key={order._id} className="hover:bg-accent-dark/10 transition-colors">
                <td className="px-8 py-6 font-serif font-bold text-secondary">
                  {order._id}
                  </td>
                ...
                <td className="px-8 py-6">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${order.orderStatus === 'completed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                    }`}>
                    {order.orderStatus}
                  </span>
                </td>
                <td className="px-8 py-6 font-serif text-sm text-secondary">
                  {order.orderAmount}
                  </td>
                <td className="px-8 py-6">
